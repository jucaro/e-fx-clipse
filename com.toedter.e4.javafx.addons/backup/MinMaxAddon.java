...
public class MinMaxAddon {
	...
	private final EventHandler widgetListener = new EventHandler() {
		...
		public void handleEvent(Event event) {
			...
			final CTabPane cTabPane = getCTFFor(changedElement);
			if (cTabPane == null) {
				return;
			}
			...
			cTabPane.setMaximizedHandler(handler);
		}
	};
	...
	private final EventHandler perspectiveChangeListener = new EventHandler() {
		@Override
		public void handleEvent(Event event) {
			final MUIElement changedElement = (MUIElement) event.getProperty(EventTags.ELEMENT);
	...
	/**
	 * If a perspective ID changes fix any TrimStacks that reference the old id
	 ...
	 */
	private final EventHandler idChangeListener = new EventHandler() {
		@Override
		public void handleEvent(Event event) {
	...
	private final EventHandler perspectiveRemovedListener = new EventHandler() {
		@Override
		public void handleEvent(Event event) {
			final MUIElement changedElement = (MUIElement) event.getProperty(EventTags.ELEMENT);
			if (!(changedElement instanceof MPerspectiveStack)) {
	...
	void hookListeners() {
		...
		topic = UIEvents.buildTopic(UIEvents.ElementContainer.TOPIC, UIEvents.ElementContainer.CHILDREN);
		eventBroker.subscribe(topic, perspectiveRemovedListener);
		topic = UIEvents.buildTopic(UIEvents.ElementContainer.TOPIC, UIEvents.ElementContainer.SELECTEDELEMENT);
		...
		topic = UIEvents.buildTopic(UIEvents.ApplicationElement.TOPIC, UIEvents.ApplicationElement.ELEMENTID);
	...
	private void setCTFButtons(CTabPane ctf, MUIElement stateElement, boolean hideButtons) {
		if (hideButtons) {
	...
	private void adjustCTFButtons(MUIElement element) {
		...
		CTabPane ctf = getCTFFor(element);
		if (ctf == null) {
			return;
		}
	...
	private CTabPane getCTFFor(MUIElement element) {
	...
	private void createTrim(MUIElement element) {
		...
		if (trimStack == null) {
			...
			Rectangle winBounds = new Rectangle(stage.getX(), stage.getY(), stage.getWidth(), stage.getHeight());
			double winCenterX = winBounds.getWidth() / 2;
			Node stackCtrl = (Node) element.getWidget();
			Bounds stackBounds = stackCtrl.getBoundsInParent(); // TODO
																// check!!!!
			double stackCenterX = stackCtrl.getLayoutX() + (stackBounds.getWidth() / 2.0);
			...
			if (bar.getWidget() == null) {
				...
				// create the widget
				context.get(IPresentationEngine.class).createGui(bar, stage, window.getContext());
			}
		} else {
			// get the parent trim bar, see bug 320756
			...
			if (parent.getWidget() == null) {
				...
				// create the widget
				context.get(IPresentationEngine.class).createGui(parent, stage, window.getContext());
			}
