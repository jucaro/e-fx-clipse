<xsl:transform version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:str="http://exslt.org/strings" xmlns:svg="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:fx="http://javafx.com/fxml" xmlns:math="http://exslt.org/math" extension-element-prefixes="math">
        ...
        <transforms>
            <xsl:for-each select="str:split($transform, ';')">
                ...
                <xsl:if test="substring-before(.,'(')='matrix'">
                    ...
                    <xsl:choose> 
                        <xsl:when test="contains(substring-before(substring-after(.,'matrix('),')'),',')">
                        ...
                        <xsl:if test="contains($style,'font-family')">
                            ...
                            <xsl:attribute name="name">
                                <xsl:choose>
                                    ...
                                    <xsl:otherwise>
                                        <xsl:value-of select="substring-after($style,'font-family:')"/>
                                    </xsl:otherwise>
                                </xsl:choose>
                            </xsl:attribute>
                        </xsl:if>
                            ...
                            <xsl:if test="contains($style,'stroke-opacity:')">
                                <opacity>
                                    <xsl:choose>
                                        ...
                                        <xsl:otherwise>
                                            <xsl:value-of select="substring-after($style,'stroke-opacity:')"/>
                                        </xsl:otherwise>
                                    </xsl:choose>
                                        ...
                                        <opacity>
                                            <xsl:choose>
                                                <xsl:when test="contains(substring-after(@style,'stop-opacity:'),';')">
                                                    <xsl:value-of select="substring-before(substring-after(@style,'stop-opacity:'),';')"/>
                                                </xsl:when>
                                                <xsl:otherwise>
                                                    <xsl:value-of select="substring-after(@style,'stop-opacity:')"/>
                                                </xsl:otherwise>
                                            </xsl:choose>
                                        </opacity>
                                    </xsl:when>
                    ...
                    <xsl:choose>
                        <xsl:when test="contains($gradient/@gradientTransform,'translate(')">
                            <xsl:variable name="tx">
                                <xsl:value-of select="substring-before( substring-after($gradient/@gradientTransform,'translate('),',')"/>
                            </xsl:variable>
                            <xsl:variable name="ty">
                                <xsl:value-of select="substring-before( substring-after($gradient/@gradientTransform,','),')')"/>
                            </xsl:variable>
                            1,0,0,1,
                        ...
                        <xsl:when test="contains($gradient/@gradientTransform,'scale(')">
                            <xsl:variable name="mxx">
                                <xsl:value-of select="substring-before( substring-after($gradient/@gradientTransform,'scale('),',')"/>
                            </xsl:variable>
            ...
            <xsl:if test="$gradient/@cx !=''">
                ...
                <xsl:element name="focusAngle">
                    <xsl:variable name="atan">
                        <xsl:value-of select="math:atan2(number($y2)-number($y1), number($x2)-number($x1))"/>
                    </xsl:variable>
                    <!--(y2 - y1), (x2 - x1)) * 180 / (3.14159265358979323846)number($atan) * 180 * -->
                
                    <xsl:value-of select="number($atan)*180*0.31830989"/>
            
                </xsl:element>
                <xsl:element name="focusDistance">
                    <!--(((x2 - x1) * (x2 - x1)) + ((y2 - y1) * (y2 - y1)))-->
                    <xsl:value-of select="math:sqrt(((number($x2)-number($x1))*(number($x2)-number($x1)))+((number($y2)-number($y1))*(number($y2)-number($y1))))"/>
                </xsl:element>
